...
class EmployeeRegisterViewSet(viewsets.ModelViewSet):
    ...
    # users/api/empRegist/chk_username/
    @action(detail=False, methods=["post"])
    def chk_username(self, request):
        try:
            if not request.data["username"]:
    ...
    def get_allowIP(self, request):
        try:
            allowIPID = request.data["allowIP"]
            userID = request.data["userID"]
            for id in allowIPID:
                IPInstance = IPRegister.objects.get(id=id)
                user = Employee.objects.get(id=userID)
                user.allowedIP.add(IPInstance)
            return JsonResponse({"msg":"IP 설정이 완료"}, status=200, safe=False)
        except:
            return JsonResponse({"msg":"IP 설정 실패"}, status=400, safe=False)
...
# ipify API를 통해 현재 IP 정보를 가져오는 함수
@api_view(["GET"])
def getIP(request):
    # ip = '8.8.8.8' 
    api_key = 'at_wUKUMkFwYQuYo7D9P7V9J9Ue1O3As'
