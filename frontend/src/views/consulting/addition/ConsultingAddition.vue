  ...
  <v-container style="max-width: none; padding:30px 0;">
    <v-row style="height:100%; display:flex; justify-content: space-between;" class="inner">
      ...
      <v-col style="padding:0; width:870px !important; flex-basis: unset; flex-grow:unset;">
        <v-card class="card-border-radius max-height-none card-shadow" outlined style="height:600px;">
          <!-- 컴포넌트 전환 영역 -->
          <div>
            <component
              ref="player"
...
<script>
import PlayerList from "/src/views/consulting/addition/PlayerList";
import CreatePlayer from "/src/views/consulting/addition/CreatePlayer";

export default {
  components: { PlayerList, CreatePlayer, },
  async created() {
  ...
  data: () => ({
    subTabs: [
      { idx: 0, componentName: "PlayerList", name:"선수관리" },
      { idx: 1, componentName: "CreatePlayer", name:"신규선수등록" },
    ],
    seltab: { idx: 0, componentName: "PlayerList", name:"선수관리" },
  }),
  methods:{
    loadingCover(val){
    ...
    consultingSettingDialog(state,id){
      this.$emit('consultingSettingDialog',state);
      this.$emit('selectAccountProfile', id)
    },
    settingPlayer(){
      this.$refs.player.getPlayer();
      this.$refs.player.getProfile();
    },
  },
  computed: {
    idx: {
