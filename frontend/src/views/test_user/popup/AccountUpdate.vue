  ...
  <div style="padding-bottom:50px;">
      <div class="manage-val3-wrap" style="
              ...
              <div class="manage-val2-top">
                ...
                <div class="manage-value2-top-inputBox">
                  ...
                  <v-btn-toggle
                    ...
                    multiple
                  >
                    <v-btn value="24" class="px-0" style="pointer-events: none;">월</v-btn>
                    <v-btn value="25" class="px-0" style="pointer-events: none;">화</v-btn>
                    <v-btn value="26" class="px-0" style="pointer-events: none;">수</v-btn>
                    <v-btn value="27" class="px-0" style="pointer-events: none;">목</v-btn>
                    <v-btn value="28" class="px-0" style="pointer-events: none;">금</v-btn>
                    <v-btn value="29" class="px-0" style="pointer-events: none;">토</v-btn>
                    <v-btn value="30" class="px-0" style="pointer-events: none;">일</v-btn>
                  </v-btn-toggle>
                </div>
          ...
          <div class="update-btnBox">
            ...
            <v-btn class="update-btn2 pre-400" @click="signup(), closeProfilePopup(), scrollTop()">저장하기</v-btn>
          </div>
...
export default {
  async created() {
    ...
    this.userID = this.$store.state.userStore.id; //생성자 id
    this.data.createUser = this.$store.state.userStore.id; //생성자 id
    this.data.comID = this.$store.state.userStore.comID; //기업 id
    // 공통데이터 가져오기
    await this.$axios
  ...
  methods: {
    ...
    scrollTop(){
      document.getElementById('playerProfile').scrollTo(0,0)
    },
    preview_image() {
    ...
    async settingAccount(id){
      if(!id){
        ...
        this.selectProfileUpdateUser = id;
        //계정 정보 세팅
        await this.$axios
          .get("users/api/empRegist/"+id+"/")
          .then((res) => {
          this.data= {
            id:res.data.id,
            username:res.data.username,
            createUser:res.data.createUser,
            modifyUser:res.data.modifyUser,
            ...
            userZip:res.data.userZip, //우편번호
            userAddr1:res.data.userAddr1,
            userAddr2:res.data.userAddr2,
            career:res.data.career, //운동경력
            awards:res.data.awards, //수상경력
            height:res.data.height,
            weight:res.data.weight,
            school:res.data.school,
            guardianName:res.data.guardianName,
            guardianNum:res.data.guardianNum,
            comID:res.data.comID,
            //attType:res.data.attType,
            sportType:res.data.sportType,
            teamID:res.data.teamID,
            disabilityType:res.data.disabilityType,
            disabilityGrade:res.data.disabilityGrade,
            education:res.data.education,
          }

          //선수의 훈련일정 가져오기
          this.$axios
          .get("attendances/api/attType/"+res.data.attType+"/")
          .then((attRes) => {
            console.log(attRes)
    ...
    validId() {
      ...
      if (!this.data.username) {
        ...
        this.err.basicMessage = "아이디는 5~20자의 영문 소문자, 숫자만 사용 가능합니다."
      } else {
        this.$axios
    ...
    basicValid(){ //아이디부터 비밀번호까지 순서대로 유효성 체크하는 함수
      ...
      else if(checkNum1.test(this.data.password) == false || checkSpc.test(this.data.password) == false || checkNum2.test(this.data.password) == false) {
        this.err.basicMessage = "비밀번호는 8자 이상, 특수문자, 숫자를 포함하여야 합니다."
      }
      ...
      else if(this.idValidFlag == false) this.err.basicMessage = "아이디 중복검사가 필요합니다."
      else if(this.data.username && this.data.name && this.data.password && this.data.passwordValid && this.idValidFlag){
    ...
    attValid(){//근무유형 이름부터 
      ...
      if(!this.attData.name) this.err.attMessage = "근무유형 이름은 필수입니다."
      else if(!this.attData.onTime) this.err.attMessage = "출근시간은 필수입니다."
      else if(!this.attData.offTime) this.err.attMessage = "퇴근시간은 필수입니다."
      else if(!this.attData.workDay) this.err.attMessage = "근무 요일을 선택해주세요"
      else if((this.offTime - this.onTime)*this.attData.workDay.length <= 0) this.err.attMessage = "출퇴근 시간과 근무요일을 확인해주세요"
      else if(this.attData.name && this.attData.onTime && this.attData.offTime && this.attData.workDay && this.attData.monthWorkTime){
        this.err.attMessage = ""
        this.attValidFlag = true
      }
    },
