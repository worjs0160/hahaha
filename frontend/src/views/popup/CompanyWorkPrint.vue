<template>
    ...
    ">
      ...
      <print2 :formTitle="formTitle" :printData="toPrintItems" :selectedUser="selectedUser" class="print-visible"></print2>
        <div class="manage-val3-header" style="display:flex; align-items:center; justify-content:space-between;">
        <h1 class="pre-400">훈련현황 출력</h1>
        <button  @click="dialogCoverSet(false), (companyWorkPrintDialogStateChange(false))">
      ...
      <!--이름 입력 끝-->
        ...
        <div class="print-btnBox">
            <v-btn class="update-btn2 pre-400" @click="dialogCoverSet(false), (companyWorkPrintDialogStateChange(false), print2())">출력</v-btn>
        </div>
        </div>
...
import Print2 from "@/components/forms/WorkPrint.vue";
    ...
    methods:{
        ...
        async print2() {
          if(await this.getUserAttData()){
            ...
            const print_sheet = document.querySelector(".print-area2");
        ...
        async getUserAttData(){
          if(this.selectedUser == ""){
            ...
            let data = {
              ...
              workDate__range:this.startDate+"T00:00:00,"+this.endDate+"T00:00:00",
            }
            
            await this.$axios({
              method: "GET",
              url: "trainings/api/CCUserTrainings",
              params: data,
            })
            .then((res) => {
